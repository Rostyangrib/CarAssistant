...
При голосовом вводе приблизительно распознанное имя исполнителя дополнительно сравнивается с именами из локального музыкального индекса. Например, `Nine Wives`, `9-майс` или `9 М` могут быть сопоставлены с `9mice`, если в каталоге нет близкого конкурирующего варианта. При слабом или неоднозначном совпадении музыка не запускается.

Плеер формирует нативную очередь Windows Media Player. После завершения трека следующий элемент очереди запускается автоматически. Для исполнителя очередь состоит из его треков; при запуске отдельного или случайного трека очередь дополняется остальной локальной библиотекой.
