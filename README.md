...
В режиме `--voice` приложение ждёт Enter и явно показывает `Listening...`. Повторный Enter немедленно завершает запись; без него запись автоматически закончится примерно после секунды тишины или по максимальному таймауту. Затем распознанный текст передаётся в тот же `Assistant.handle`, что используется текстовым CLI. После ответа цикл повторяется; `Ctrl+C` завершает работу. Wake Word, постоянное прослушивание и голосовой ответ на этом этапе отсутствуют.
